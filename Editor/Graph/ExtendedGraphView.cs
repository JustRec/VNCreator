...
namespace VNCreator.Editors.Graph
{
    ...
    public class ExtendedGraphView : GraphView
    {
        ...
        public void GenerateNode(string _nodeName, Vector2 _mousePos, int choiceAmount, bool _startNode, bool _endNode, bool _flag)
        {
            AddElement(CreateNode(_nodeName, _mousePos, choiceAmount, _startNode, _endNode, _flag));
        }

        public BaseNode CreateNode(string _nodeName, Vector2 _mousePos, int choiceAmount, bool _startNode, bool _endNode, bool _flag)
        {
            return CreateNode(_nodeName, _mousePos, choiceAmount, new string[choiceAmount].ToList(), _startNode, _endNode, _flag, new NodeData());
        }

        public BaseNode CreateNode(string _nodeName, Vector2 _mousePos, int choiceAmount, List<string> _choices, bool _startNode, bool _endNode, bool _flag, NodeData _data)
        {
            ...
            if (!_endNode)
            {
                if (choiceAmount > 1)
                {
                    ...
                    {
                        Port _outputPort = CreatePort(_node, Direction.Output, Port.Capacity.Single); //Do not change the single whatsoever
